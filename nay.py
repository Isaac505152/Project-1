...
for entryNum in range(nEntries): 
    ...
    # Retrieve particle and jet attributes
    ...
    for i in range(len(pt)):
        if abs(iddi[i]) == 5:  # Identify b-quarks
            print(f"Identified b-quark with pt: {pt[i]}, eta: {eta[i]}, phi: {phi[i]}")
            
            part = ROOT.TLorentzVector()
            part.SetPtEtaPhiM(pt[i], eta[i], phi[i], 4.18)  # Assuming 4.18 GeV as b-quark mass
    ...
    # Fill the b-quark count histogram
    hist_bquark_count.Fill(bquark_count)
    print(f"Filling bquark_count histogram with: {bquark_count}")

    # If exactly 4 b-quarks are found, fill the invariant mass histogram
    if bquark_count == 4:
        bquark_system = bquark[0] + bquark[1] + bquark[2] + bquark[3]
        bquarkMass = bquark_system.M()
        hist_bquark_mass.Fill(bquarkMass)
        print(f"Filling bquark_mass histogram with mass: {bquarkMass}")

    # Process and fill jet histograms
    if len(jet_pt) >= 4: 
